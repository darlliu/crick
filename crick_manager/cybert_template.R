...
raw.data <- read.table(fname, sep = '\t', header = F)
...
ans <- bayesT(d, numC, numE, winSize = winSize, conf = conf, bayesIntC=F, bayesIntE=F, doMulttest=T, ppde=F)
...
write.table(wd, "cybert.out", sep='\t', row.names=F, quote=F)
